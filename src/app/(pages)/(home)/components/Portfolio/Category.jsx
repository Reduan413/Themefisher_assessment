...
const Category = ({ categories, activeCategory, setActiveCategory }) => {
  return (
    <div
      className="mb-8 mt-5 lg:mt-10 overflow-x-auto rounded-md bg-[#000000]"
      style={{
        boxShadow: "0px 0px 25px 10px #FFFFFF0F",
      }}
    >
      <div className="flex gap-2 lg:gap-5 rounded-md px-3 lg:px-10">
        {categories.map((category) => (
          <button
            key={category}
            onClick={() => setActiveCategory(category)}
            className={cn(
              "relative whitespace-nowrap px-2 py-3 lg:p-5 text-sm lg:text-base",
